...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="h-16 border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#0c0e14] px-3 sm:px-6 flex items-center justify-between sticky top-0 z-10 select-none no-print transition-colors shadow-xs">
      ...
      <div className="flex items-center gap-4 flex-1 max-w-md">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full bg-slate-100 dark:bg-[#141722] text-slate-900 dark:text-zinc-100 placeholder-slate-500 dark:placeholder-zinc-400 pl-10 pr-14 py-2 min-h-[40px] rounded-lg border border-slate-300 dark:border-zinc-700 focus:border-[#fe7518] focus:bg-white dark:focus:bg-[#161a26] focus:outline-none focus:ring-2 focus:ring-[#fe7518]/30 transition-all font-sans text-xs touch-manipulation font-medium"
          />
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
            {searchQuery ? (
              <button 
                ...
                className="text-xs text-slate-700 hover:text-slate-950 dark:text-zinc-300 dark:hover:text-white font-bold px-2 py-0.5 rounded hover:bg-slate-200 dark:hover:bg-zinc-800 transition-colors"
              >
                Clear
              </button>
            ) : (
              <kbd className="hidden sm:inline-block text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-slate-200 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 border border-slate-300 dark:border-zinc-700">
                ⌘K
              </kbd>
            )}
          </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-[#131620] border border-slate-300 dark:border-zinc-700 text-xs font-semibold">
          ...
          <span className="capitalize text-slate-900 dark:text-zinc-100">{currentRole}</span>
        </div>

        {/* Light / Dark Theme Switcher Button */}
        <button
          onClick={toggleTheme}
          className="p-2 rounded-lg bg-slate-100 dark:bg-[#131620] hover:bg-slate-200 dark:hover:bg-zinc-800 border border-slate-300 dark:border-zinc-700 text-slate-800 dark:text-zinc-200 btn-haptic"
